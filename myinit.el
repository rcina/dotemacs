...
(use-package consult
  ...
  (consult-customize
   ...
   ;; 2. The Settings (Keys and Values)
   :preview-key '(:debounce 0.4 any)))
...
(use-package scss-mode
  ...
  :mode ("\\.scss\\'" . scss-mode)
  :config
  (setq scss-compile-at-save nil))  ; Disable automatic compilation on save
...
(use-package auctex
  :straight t
  :config
  ;; Enable TeX-fold-mode for better readability of LaTeX code
  (setq TeX-fold-mode-default-list '(("\\section\\*?" TeX-fold-section)))
  (add-hook 'latex-mode-hook #'TeX-fold-mode)

  ;; Configure AUCTeX to use a specific LaTeX compiler if needed.
  (setq TeX-engine 'xetex) ;; or 'luatex, 'pdflatex, etc.
  (setq TeX-command-extra-options "")

  ;; Set the viewer for PDF files.
...
(use-package pdf-tools
  ...
  :init
  ;; Add the `pdf-tools` server to your exec path
  (pdf-tools-install)
  :config
  (define-key pdf-view-mode-map (kbd "C-c C-o") 'pdf-tools-open-file)
  (setq pdf-view-auto-refresh-on-file-change t)
  ;; Automatically open a new PDF view when compiling a document.
  (add-hook 'TeX-after-compilation-finished-functions
            (lambda (program)
              (when (string-equal program "latexmk")
                (let* ((source-file buffer-file-name)
                       (pdf-file (TeX-output-file-name TeX-master-file-name)))
                  (if (file-exists-p pdf-file)
                      (progn
                        (message "Opening PDF file: %s" pdf-file)
                        (find-file pdf-file))
                    (message "Could not find PDF file: %s" pdf-file)))))))

;; --- Tab Completion with Yasnippet ---
(use-package yasnippet
  :straight t
  :init
  (yas-global-mode 1))

;; Install the official snippet collection.
(use-package yasnippet-snippets
  :straight t
  :after yasnippet)

;; Optional: Customize the key for snippet expansion
(setq yas-trigger-key "TAB")

;; Make sure yasnippet is fully loaded before trying to use it.
;; A simple hook can sometimes be enough.
