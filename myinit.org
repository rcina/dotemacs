...
*** Kill Ring
=M-y= is remapped to =consult-yank-pop= (configured in the Consult use-package).
...
*** Language: Python — Ruff (linter) + Apheleia/Black (formatter)
Apheleia handles formatting via black (configured in the Miscellaneous
Programming Tools section). Ruff handles linting via flycheck.
